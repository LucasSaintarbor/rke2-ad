...
[TIP]
====
If your node has NetworkManager installed and enabled, xref:../known_issues.adoc#networkmanager[ensure that it is configured to ignore CNI-managed interfaces]. If your node has Wicked installed and enabled, xref:../known_issues.adoc#wicked[ensure that the forwarding sysctl config is enabled].
====
