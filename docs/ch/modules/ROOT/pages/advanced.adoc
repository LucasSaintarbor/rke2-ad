...
`config.toml.tmpl` 是一个 Go 模板文件，并且 `config.Node` 结构会被传递给模板。有关如何使用该结构自定义配置文件的示例，请参见https://github.com/k3s-io/k3s/blob/master/pkg/agent/templates/templates.go#L16-L32[此模板]。
...
请注意，由于 server Chart 对集群的操作非常重要，因此集群操作人员需要确保已禁用或更换组件。有关集群中各个系统 Chart 角色的更多信息，请参阅link:./architecture.adoc#server-charts[架构概述]。
...
. 确保满足所有常见的 AWS 云提供商https://rancher.com/docs/rke/latest/en/config-options/cloud-providers/aws/[先决条件]。它们与区域无关，并且始终是必需的。
...
如果你使用的是https://docs.aws.amazon.com/vpc/latest/privatelink/endpoint-services-overview.html[私有 AWS 端点]，请确保为每个私有端点使用适当的 `URL`。
