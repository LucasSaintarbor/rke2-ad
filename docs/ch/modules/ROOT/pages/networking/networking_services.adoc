...
CoreDNS is deployed with the https://github.com/kubernetes-incubator/cluster-proportional-autoscaler[autoscaler] by default. To disable it or change its config, use the link:../helm.adoc#使用-helmchartconfig-自定义打包组件[HelmChartConfig] resource.
...
Configuration options can be specified by creating a link:../helm.adoc#使用-helmchartconfig-自定义打包组件[HelmChartConfig manifest] to customize the `rke2-ingress-nginx` HelmChart values. For example, a HelmChartConfig at `/var/lib/rancher/rke2/server/manifests/rke2-ingress-nginx-config.yaml` with the following contents sets `use-forwarded-headers` to `"true"` in the ConfigMap storing the NGINX config:
